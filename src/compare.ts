...
export function calculateTotalRow(data: Array<CompareResult>): CompareResult {
	...
	totalRow.remark =
		totalRow.bytes === totalRow.baseBytes
			? "no-change"
			: totalRow.bytes > totalRow.baseBytes
				? "increased"
				: "decreased";
...
function renderNote(d: CompareResult, redThreshold: number): string {
	...
	if (d.baseBytes === 0) {
		// Total row will fall here when all bundles are swapped (deleted and added).
		// See https://github.com/exoego/esbuild-bundle-analyzer/issues/90
		return "✅  No change";
	}
